    ...
    public class MembershipDAL
    {
        ...
        public static async Task<int> CreateAsync(Membership memberships)
        {
            ...
            using (var dbContext = new ContextDB())
            {
                ...
                {
                    dbContext.Membership.Add(memberships);
        ...
        public static async Task<int> UpdateAsync(Membership memberships)
        {
            ...
            using (var dbContext = new ContextDB())
            {
                var membershipDB = await dbContext.Membership.FirstOrDefaultAsync(m => m.Id == memberships.Id);
        ...
        public static async Task<List<Membership>> GetAllAsync()
        {
            ...
            using (var dbContext = new ContextDB())
            {
                memberships = await dbContext.Membership.ToListAsync();
            }
            return memberships;
        }
        #endregion
        ...
        {
            bool result = false;
            var memberships = await contextDB.Membership.FirstOrDefaultAsync(m => m.Dui == membership.Dui && m.Id != membership.Id);
